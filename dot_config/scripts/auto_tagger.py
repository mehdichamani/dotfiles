...
try:
    import mutagen
    from mutagen.id3 import ID3, TIT2, TPE1, TALB, TDRC, TRCK, TCON, APIC, USLT, error as ID3Error
...
def process_file(file_path: Path, args: argparse.Namespace) -> Optional[Path]:
    ...
    print(f"  {Colors.DIM}🔍 Search Query:{Colors.RESET} '{query}'")
    ...
    print(f"    {Colors.BOLD}Album:{Colors.RESET}   {album} ({year})")
    print(f"    {Colors.BOLD}Genre:{Colors.RESET}   {genre} | {Colors.BOLD}Track:{Colors.RESET} {track_str}")
...
def main():
    ...
    if target.is_file():
        ...
        result_path = process_file(target, args)
