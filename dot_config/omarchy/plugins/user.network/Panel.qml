...
Panel {
  ...
  property string focusSection: "dns"  // "header" | "band" | "dns" | "wireguard" | "wifi"
  property int headerIndex: 0
  property int wgIndex: 0
  readonly property bool canDisconnect: !!connectedWifiNetwork
  readonly property bool headerHasDisconnect: false
  readonly property bool canShareWifi: info.type === "wifi" && canShareNetwork(connectedWifiNetwork)
  readonly property bool canToggleWired: !!wiredDevice || info.type === "ethernet"
  ...
  readonly property int wiredHeaderIndex: canToggleWired ? (canShareWifi ? 1 : 0) + (canRunSpeedTest ? 1 : 0) : -1
  readonly property int toggleHeaderIndex: canToggleWifi ? (canShareWifi ? 1 : 0) + (canRunSpeedTest ? 1 : 0) + (canToggleWired ? 1 : 0) : -1
  readonly property int headerActionCount: (canShareWifi ? 1 : 0) + (canRunSpeedTest ? 1 : 0) + (canToggleWired ? 1 : 0) + (canToggleWifi ? 1 : 0)
  ...
  readonly property string wiredToggleHint: wiredConnected ? "Turn Ethernet off" : "Turn Ethernet on"
  ...
  function toggleWired() {
    var dev = root.wiredIfaceName || "enp5s0"
    var isUp = root.wiredConnected
    root.pendingWiredAction = isUp ? "down" : "up"
    wireguardActionProc.command = ["bash", "-c", isUp ? ("nmcli device disconnect " + dev + " || nmcli connection down 'Wired connection 1'") : ("nmcli device connect " + dev + " || nmcli connection up 'Wired connection 1'")]
      ...
      onMoveRequested: function(dx, dy) {
        ...
        if (dy !== 0) {
          ...
          if (root.focusSection === "header") {
            ...
            // then the header; otherwise stays put. j drops into wireguard or wifi list.
            if (dy < 0) {
              if (root.canSelectBand) {
                root.focusSection = "band"
