...
BarWidget {
  ...
  component IndicatorLoader: Item {
    ...
    function injectProps() {
      ...
      if ("fontSize" in target) target.fontSize = Style.font.heading
      if ("slotSize" in target) target.slotSize = Style.bar.iconSlot
      if ("fixedWidth" in target) target.fixedWidth = root.vertical ? -1 : Style.bar.iconSlot
      if ("fixedHeight" in target) target.fixedHeight = root.vertical ? Style.bar.iconSlot : -1
    }
